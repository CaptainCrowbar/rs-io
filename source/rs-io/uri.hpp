...
namespace RS::IO {
    ...
    public:
        ...
        std::string scheme() const { return empty() ? std::string() : Format::ascii_lowercase(text_.substr(0, ofs_user_ - (has_slashes() ? 3 : 1))); }
...
namespace std {
    ...
    template <>
    class hash<RS::IO::Uri> {
        size_t operator()(const RS::IO::Uri& u) const noexcept {
            return u.hash();
        }
    };
